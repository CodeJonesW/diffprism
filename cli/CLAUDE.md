...
- For a PR, the server starts a headless, read-only Claude Code agent that answers the reviewer's comments until the review is submitted — for PRs opened from the dashboard too (`src/commands/pr-agent.ts`, #217, #224). The command returns once the review is open and prints the `claude --resume` command for the agent's conversation. `--no-agent` — open the PR review without starting one. If the server can't start one (no `claude` on its PATH), it prints the prompt to paste instead.
...
- `src/commands/pr-agent.ts` — The agent that answers comments on a PR review: waits for questions, runs `claude -p` one turn at a time. `prAgentStarter()` is what `diffprism server` hands the server to start one per PR review
